...
export default function PlantDetail() {
  ...
  const pickJournalPhoto = async () => {
    ...
    if (!result.canceled && result.assets[0].base64) {
      setJournalPhoto(result.assets[0].base64);
    }
  };
  ...
  const deleteLog = async (logId: string) => {
    try {
      await api.del(`/health-logs/${logId}`);
      fetchData();
    } catch (e: any) {
      Alert.alert('Error', e.message);
    }
  ...
  if (!plant) {
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.center}><Text style={styles.emptyText}>Plant not found</Text></View>
      </SafeAreaView>
    );
  }
  ...
  const careItems = [
    { icon: 'water-outline', label: 'Watering', value: care.watering || `Every ${plant.watering_frequency_days || 7} days`, family: 'Ionicons' },
    { icon: 'sunny-outline', label: 'Sunlight', value: care.sunlight || 'Bright indirect', family: 'Ionicons' },
    { icon: 'water', label: 'Humidity', value: care.humidity || 'Moderate', family: 'Ionicons' },
    { icon: 'thermometer-outline', label: 'Temperature', value: care.temperature || '60-75°F', family: 'Ionicons' },
    { icon: 'leaf-outline', label: 'Soil', value: care.soil || 'Well-draining', family: 'Ionicons' },
    { icon: 'flask-outline', label: 'Fertilizer', value: care.fertilizer || 'Monthly', family: 'Ionicons' },
  ];
            ...
            healthLogs.map(log => (
              <View key={log.id} style={styles.logCard}>
                <View style={styles.logHeader}>
                  ...
                  <TouchableOpacity testID={`delete-log-${log.id}`} onPress={() => deleteLog(log.id)}>
                    <Feather name="x" size={16} color={Colors.textTertiary} />
                  </TouchableOpacity>
                </View>
                <Text style={styles.logNotes}>{log.notes}</Text>
                {log.photo && (
                  <Image source={{ uri: `data:image/jpeg;base64,${log.photo}` }} style={styles.logPhoto} contentFit="cover" />
                )}
              </View>
            ))
          )}
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              ...
              <TouchableOpacity testID="close-journal-modal" onPress={() => setShowJournalModal(false)}>
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              ...
              <TouchableOpacity testID="close-edit-modal" onPress={() => setShowEditModal(false)}>
                <Feather name="x" size={24} color={Colors.textPrimary} />
              </TouchableOpacity>
            </View>
            ...
            <TextInput testID="edit-location-input" style={styles.editInput} value={editLocation} onChangeText={setEditLocation} placeholderTextColor={Colors.textTertiary} />
...
const styles = StyleSheet.create({
  ...
  heroOverlay: { position: 'absolute', bottom: 0, left: 0, right: 0, height: 100, backgroundColor: 'transparent' },
  ...
  actionBtnPrimary: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: Colors.primary, height: 48, borderRadius: Radius.pill, shadowColor: Colors.primary, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8, elevation: 8 },
  ...
  primaryBtn: { backgroundColor: Colors.primary, height: 52, borderRadius: Radius.pill, justifyContent: 'center', alignItems: 'center', shadowColor: Colors.primary, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8, elevation: 8 },
